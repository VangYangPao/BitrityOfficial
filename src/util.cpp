...
const char * const BITCOIN_CONF_FILENAME = "bitrity.conf";
const char * const BITCOIN_PID_FILENAME = "bitrityd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "bitrity" is a composite category enabling all Bitrity-related debug output
                if(ptrCategory->count(std::string("bitrity"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BitrityCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BitrityCore
    // Mac: ~/Library/Application Support/BitrityCore
    // Unix: ~/.bitritycore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BitrityCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BitrityCore";
#else
    // Unix
    return pathRet / ".bitritycore";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty bitrity.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override bitrity.conf
