...
{
    return QCoreApplication::translate("bitrity-core", psz).toStdString();
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in bitrity.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in bitrity.qrc)
...
/** Class encapsulating Bitrity Core startup and shutdown.
...
/** Main Bitrity application object */
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if(retval)
    {
        ...
        // bitrity: URIs or payment requests:
...
{
    QMessageBox::critical(0, "Runaway exception", BitcoinGUI::tr("A fatal error occurred. Bitrity Core can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(bitrity);
    Q_INIT_RESOURCE(bitrity_locale);
    ...
    /// 6. Determine availability of data directory and parse bitrity.conf
    ...
    if(!masternodeConfig.read(strErr)) {
        QMessageBox::critical(0, QObject::tr("Bitrity Core"),
    ...
    // bitrity: links repeatedly have their payment requests routed to this process:
