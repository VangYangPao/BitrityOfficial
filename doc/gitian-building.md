...
*Setup instructions for a Gitian build of Bitrity Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bitrity
...
- [Building Bitrity Core](#building-bitrity-core)
...
Clone the git repositories for Bitrity Core and Gitian.
...
in the Bitrity Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bitrity Core
----------------

To build Bitrity Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Bitrity Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bitrity/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bitritypay/bitrity
...
For example:
```bash
URL=https://github.com/crowning-/bitrity.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit bitrity=${COMMIT} --url bitrity=${URL} ../bitrity/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bitrity=${COMMIT} --url bitrity=${URL} ../bitrity/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bitrity=${COMMIT} --url bitrity=${URL} ../bitrity/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bitrity git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bitrity/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bitrity
SIGPATH=/some/root/path/bitrity-detached-sigs

./bin/gbuild --url bitrity=${BTCPATH},signature=${SIGPATH} ../bitrity/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bitrity-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bitrity-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bitrity-osx-build.assert
```
...
[bitrity/gitian.sigs](https://github.com/bitritypay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
