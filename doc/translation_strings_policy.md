...
This document provides guidelines for internationalization of the Bitrity Core software.
...
Plurals can be complex in some languages. A quote from the gettext documentation:
    ...
    In Bitrityh we use e.g. plik (file) this way:
    1 plik,
    2,3,4 pliki,
    5-21 pliko'w,
    22-24 pliki,
...
This can be checked by executing `make translate` in the `src` directory, then verifying that `bitrity_en.ts` remains unchanged.
