...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Bitrity database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Bitrity datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Bitrity datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Bitrity 0.7 datadir."
    ;;
  3)
    echo "Detected Bitrity pre-0.8 datadir."
    ;;
  4)
    echo "Detected Bitrity 0.8 datadir."
    ;;
esac
